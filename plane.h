...
public:
  ...
  bool operator==(const Plane &a) const
  {
    ...
    if ( dx > PLANE_EPSILON ) return false;

    float dy = (float)fabs( N.y - a.N.y );
    if ( dy > PLANE_EPSILON ) return false;

    float dz = (float)fabs( N.z - a.N.z );
    if ( dz > PLANE_EPSILON ) return false;

    float dd = (float)fabs( D - a.D );
    if ( dd > PLANE_EPSILON ) return false;
  ...
  float DistToPt(const Vector3d<float> &pos) const
  {
    return (pos.x*N.x + pos.y*N.y + pos.z*N.z)+D;
  };

  bool Intersect(const Vector3d<float> &p1,
  ...
  bool Exclude(bool &isect,const Vector3d<float> &bMin,const Vector3d<float> &bMax)
  {
    ...
    if ( DistToPt(neg) > 0) return true; // is excluded!
      else if ( DistToPt(pos) > 0) isect = true; // does intersect plane!
  ...
  {
    printf("%s N=(%.2f,%.2f,%.2f) D=(%.2f)\n",name,N.x,N.y,N.z,D);
  };

  void Print(FILE *fph,const char *name) const
  {
    fprintf(fph,"%s N=(%.2f,%.2f,%.2f) D=(%.2f)\n",name,N.x,N.y,N.z,D);
  };

  void Compute(const Vector3d<float> &A,
